...
// 0.0174533 ==> 1 degree
  ...
  while (!done) {
	...
	dConstPos(i,i) = 10; dConstOri(i,i) = 0.01;
    ...
    if (ctrlSelect == 0){
        ...
        printf ("error: cannot set force (%s)\n", dhdErrorGetLastStr());
                ...
                // PROPOSED METHOD + CONTROLLER ACTIVE (GRIPPER CASE)
                if (dhdSetForceAndTorqueAndGripperForce (FkCtrl[0] + Fmid_Ctrl[0], FkCtrl[1] + Fmid_Ctrl[1], FkCtrl[2] + Fmid_Ctrl[2], TauCtrl[0] + TauMid_Ctrl[0] + Tau_dCtrl[0], TauCtrl[1] + TauMid_Ctrl[1] + Tau_dCtrl[1], TauCtrl[2] + TauMid_Ctrl[2] + Tau_dCtrl[2], F_haptic_thumbFing[0] + F_haptic_thumbFing[1] + F_haptic_thumbFing[2]) < DHD_NO_ERROR) {
        printf ("error: cannot set force (%s)\n", dhdErrorGetLastStr());
        done = 1;
        }
    }
            ...
            // PROPOSED METHOD (DAMPING FORCE ==> FdCtrl[0] (ADD, IF NEEDED))
            if (dhdSetForceAndTorqueAndGripperForce (FkCtrl[0] + Fmid_Ctrl[0] , FkCtrl[1] + Fmid_Ctrl[1], FkCtrl[2] + Fmid_Ctrl[2], TauCtrl[0] + TauMid_Ctrl[0] + Tau_dCtrl[0], TauCtrl[1] + TauMid_Ctrl[1] + Tau_dCtrl[1], TauCtrl[2] + TauMid_Ctrl[2] + Tau_dCtrl[2], F_haptic_thumbFing[0] + F_haptic_thumbFing[1] + F_haptic_thumbFing[2]) < DHD_NO_ERROR) {
        printf ("error: cannot set force (%s)\n", dhdErrorGetLastStr());
        done = 1;
        } 
