...
if (std_lowBandwidth_timeDelay_TeleOp_FIC_Act == 0){
	if (gripperAct == 1){ 
      ...
      if (abs(gripperGapVec[0]) < 0.001){
		  std::cout << "right haptic gripper gap value: " << abs(gripperGapVec[0]) << std::endl;
...
static void mainOptoForce()
{
    ...
    std::cout << "Opening OptoForce device: " 
        << portlist[0].name << " " 
        << portlist[0].deviceName << " " 
        << portlist[0].serialNumber << std::endl;
    if (!daq.open(portlist[0])) {
...
static void mainFranka(const char* frankaIP){
    ...
    std::cout << "Franka go to initial posture..." << std::endl;
	  std::cin.ignore();
		...
		// std::array<double, 7> postureGoal = {{-0.164892, -0.339858, -0.384224, -2.37786, -0.122578, 2.10618, 0.335746}}; //-0.122578 --> 5
		
		// 45 degree ang: -0.129699 -0.228457 -0.493685  -2.17448  0.144543   1.80917  0.124129
		//std::array<double, 7> postureGoal = {{-0.129699,-0.228457, -0.493685,  -2.17448,  0.144543,   1.80917,  0.124129}}; //-0.122578 --> 5
      ...
      auto callbackControl = [&model, &timeController, &filterTargetVel, &filterErrorVel, &filterGripRatio, &targetCartPos, &desCartOriMat, &targetCartMat, &targetCartMat_FIC, &desiredJointPos, &wrenchIntegral, &gripperPressed, &splinesGotoMode, &lastGotoMode]
           (const franka::RobotState& state, franka::Duration period) -> franka::Torques
        {
         ...
         if(initial_configuration_Franka == 0 || initial_configuration_Franka == 1){
        	newTargetCartMat(1,1) = 1;
         	newTargetCartMat(1,2) = 0;
         	newTargetCartMat(2,1) = 0;
         	newTargetCartMat(2,2) = 1;
         	
         	// 45 Degree Drilling
         	//newTargetCartMat(1,1) = 0.70;
         	//newTargetCartMat(1,2) = -0.70;
         	//newTargetCartMat(2,1) = 0.70;
         	//newTargetCartMat(2,2) = 0.70;
         ...
         if (frankaIsControl.load()) {
          ...
          targetCartVel = filterTargetVel.value();
					...
					// DEMO_exp 
					// maxDisp_Franka[0] = 0.05;
					// maxDisp_Franka[1] = 0.05;
					// maxDisp_Franka[2] = 0.05;
				  ...
				  curPos3x1_Franka[2] = readCartPos(2);  
					...
					if (frankaDesPosHapticCtl == 0){
						if (initial_configuration_Franka == 0){
							...
							desPos3x1_Franka[1] = -0.249;
				 			desPos3x1_Franka[2] = +0.301;
						}else if (initial_configuration_Franka == 2){
							//INITIAL CONFIGURATION FOR BILATERAL TELEOPERATION
							...
							desPos3x1_Franka[1] = -0.250;
				 			...
				 			/*
				 			desPos3x1_Franka[0] = +0.311781;
							desPos3x1_Franka[1] = -0.362779 ;
				 			// desPos3x1_Franka[1] = -0.009 + 0.1 * sin(0.0625 * 2 * 3.14 * timeControllerGlobalVar);
				 			desPos3x1_Franka[2] = +0.60;
				 			*/
          ...
          Eigen::Vector6d errorCartVel = targetCartVel - readCartVel;
            ...
            wrenchStiffnessGlobalVar = wrenchStiffness;
              ...
              // DEMO
            	...
            	potEnergy_6x1_Franka[i+3] = (kConstOri_Franka(i,i) * pow(maxDisp_Ori_Franka[i], 2))/2 - pow(maxDisp_Ori_Franka[i], 2)/(2 * log((TauMax_Franka[i] - kConstOri_Franka(i,i) * maxDisp_Ori_Franka[i])/maxDisp_Ori_Franka[i])) - TauMax_Franka[i] * (maxDisp_Ori_Franka[i] - fabs(angular_error_Franka[i])) + (maxDisp_Ori_Franka[i] * (TauMax_Franka[i] - kConstOri_Franka(i,i) * maxDisp_Ori_Franka[i]))/(2 * log((TauMax_Franka[i] - kConstOri_Franka(i,i) * maxDisp_Ori_Franka[i])/maxDisp_Ori_Franka[i]));
						...
						//std::cout << "FIC Wrench Franka Right ==> " << Wrench_FIC_Franka_doubleType.transpose() << std::endl;	
            ...
            frankaRead4.store(wrenchStiffness(2) + wrenchIntegral(2));
            	...
            	tauCmdEigen = coriolisEigen + jacobianEigen.transpose()*(wrenchStiffness /*+ wrenchDamping + wrenchIntegral*/  + Wrench_FIC_Franka_doubleType) + NullSpaceCtrl;
							...
							//std::cout << "current: " << readCartMat << std::endl << std::endl;
							//std::cout << "dessired:  "<< newTargetCartMat << std::endl << std::endl;
							//std::cout << "Interactive Force Right: " << optoForceX.load() << "," << optoForceY.load() << "," <<  optoForceZ.load() << std::endl << std::endl;
							//std::cout << "Right Arm curJointPose: " << readJointPosGlobalVar.transpose() << std::endl << std::endl;
							//std::cout << "Right Arm curPose: " << curPos3x1_Franka.transpose() << std::endl << std::endl;
    		...
    		buffVec_Vel_OriZ[count] = velAngularError[2];
				*/
				...
				//std::cout << "Position Buffer Vector ==> " << buffVec_6x1.transpose() << std::endl;
				//std::cout << "Current Position Vector ==> " << curPos3x1_Franka.transpose() << std::endl << std::endl;

				//std::cout << readJointPos << std::endl << std::endl; 
    ...
    while (!frankaIsDone.load()) {
      if (timeControllerGlobalVar > 0){
        if (cnt > lastCnt){
          ...
          bilateral_telemanipulation_ctrl_output.open("rightArm_bilateral_telemanipulation_ctrl_output.txt", std::ios_base::app);
...
void mainGUI(){
  ...
  viewer.drawFrame(targetPosHand, Eigen::Matrix3d::Identity());
	...
	viewer.drawSphere(targetPosHand - endEffPosGroundLevelProj, 0.0075, 0.0, 0.0, 1.0);
  ...
  //viewer.drawEllipsoid(posInfo4, Eigen::Matrix3d::Identity(), 0.5*stiffnessAng, 0.4, 0.4, 1.0);
	...
	if (initial_configuration_Franka == 0){
      	...
      	frankaTargetPosLinYSphere.store(-0.249);
      	frankaTargetPosLinZSphere.store(+0.301);
      }else if(initial_configuration_Franka == 2){
      	frankaTargetPosLinXSphere.store(+0.316);
      	...
      	frankaTargetPosLinZSphere.store(+0.70);
				...
				/* 			
      	frankaTargetPosLinXSphere.store(+0.311781);
      	frankaTargetPosLinYSphere.store(-0.362779);
      	frankaTargetPosLinZSphere.store(+0.60);
      	*/
     }
  ...
  //User keyborad inputs
	...
	if (viewer.isKeyPressed(sf::Keyboard::Numpad0)) {
			//Eigen::Vector3d newTargetEndEffPosUI;
			...
			std::cout << "Right Arm Current Target End-effector Sphere Position ==> " << newTargetEndEffPosUI.transpose() << std::endl;
			std::cout << "Right Arm Franka Current End-Effector Position ==> " << curPos3x1_Franka.transpose() << std::endl;
			std::cout << "Right Arm Franka Current End-Effector Joint Position ==> " << readJointPosGlobalVar.transpose() << std::endl << std::endl; 
	}

	if (viewer.isKeyPressed(sf::Keyboard::RShift) && goToMode.load() == false){
			goToMode.store(true);
			std::cout << "Space Key Pressed? " << goToMode << std::endl << std::endl;
	}
	
	if (viewer.isKeyPressed(sf::Keyboard::RControl)){
			goToMode.store(false);
	...
	if (viewer.isKeyPressed(sf::Keyboard::F6)) {
		...
		if (viewer.isKeyPressed(sf::Keyboard::F10)) {
			inputRequestStage += 1;
		...
		if (viewer.isKeyPressed(sf::Keyboard::F9)) {
			inputRequestStage -= 1;
			std::cout << "Input Request: "  << inputRequestStage << std::endl <<std::endl;
